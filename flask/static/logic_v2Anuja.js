...
let selection = d3.select("#energy-select-anuja")
...
//populate dropdown with csv headers (text, property value)
    energy_options2.forEach(option =>{
        selection.append("option").text(option).property("value",option)
    })


// get the iso from the countries json
d3.json("static/countries.json").then(country => {
    getData2(country, energy_options2[0]);
});



}
function getData2(countries,chosen_x) {
    //console.log(countries);

    function filter_and_unpack2(rows, key, year) {
        //console.log(rows)
        return rows.filter(d => d.year == year).map(row => row[key])
    }
    d3.csv("static/cleaned_data_lat_long.csv").then(rows => {
        ...
        for (var i = 0; i <= n; i++) {
            //console.log(rows[0])
            var z = filter_and_unpack2(rows, chosen_x, num)
            var country_name = filter_and_unpack2(rows, 'country', num)
            //console.log(z)
        ...
        let index = energy_options2.indexOf(chosen_x);
        console.log(index);
        colorscales2 = ["Blues","Greens", "Blues"]
        units2 = ["Mt", "bcm", "Mt"]
        let colorscale2 = colorscales2[index];
        let unit2 = units2[index];
...
init2();
function optionchange2(chosen_x){
    d3.json("static/countries.json").then(country => {
        getData2(country, chosen_x)});
}
